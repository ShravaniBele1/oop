...
int main()
{
    Bank bank;
    bank.deposit(9000);
    bank.deposit(2500.75f);
    bank.deposit("gpay@upi");

    Account acc1(1, 20000);
    Account acc2(2, 6000);

    acc1.display();
    acc2.display();

    transfer(acc1, acc2, 4000);  // Transfer money

    acc1.display();
    acc2.display();

    acc1 = acc1 - 3000;  // Withdraw money
    cout << "\nWithdrawing Rupees " << 3000 << " from Account 1..." << endl;
    acc1.display();
